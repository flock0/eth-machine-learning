...
if __name__=='__main__':
    ...
    unionLR = FeatureUnion([('kernelpca', KernelPCA(kernel='linear'))])
    pipeLR = Pipeline([('featureunion', unionLR), ('lr', lr)])
    ...
    percentile = np.array([1, 15, 20, 25])
    scoreFunc = [f_classif]
    lrC = np.array([0.001, 0.01, 0.1, 0.25])
    svmC = np.array([0.001, 0.005, 0.1, 0.2])
    svmGamma = np.array([0.00025, 0.0001, 0.000075])
    ...
    rfNEstimators = np.array([40, 75, 100, 125, 200])
    ...
    grid = GridSearchCV(estimator=pipeLR, param_grid=dict(lr__C=lrC), scoring="accuracy", cv=FOLDS)
    ...
    # print("GBC...")
    # grid = GridSearchCV(estimator=pipeGB, param_grid=dict(gbc__loss=gbLoss, gbc__max_depth=gbMaxDepth,gbc__learning_rate=gbLearningRate, gbc__n_estimators=gbNEstimators, featureunion__kernelpca__kernel=kernelPCAKernel, featureunion__kbest__k=percentile, featureunion__perc__score_func=kBestScoreFunc), scoring="accuracy", cv=FOLDS)
    # grid.fit(features[:TRAIN_SIZE,:], labels[:TRAIN_SIZE])
